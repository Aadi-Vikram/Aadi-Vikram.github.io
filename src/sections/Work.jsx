    ...
    >
      ...
      <div>
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
          <h3 className="display text-[1.4rem] sm:text-[1.65rem] m-0">{job.company}</h3>
          {job.current && (
            <span
        ...
        <p className="text-[13.5px] font-medium mt-1.5 mb-3" style={{ color: "var(--accent)" }}>
          {job.role}
        </p>

        <p className="text-[14px] leading-relaxed m-0 mb-4 max-w-[62ch]" style={{ color: "var(--muted)" }}>
          {job.summary}
        </p>
...
export default function Work() {
  return (
    <section id="work" className="max-w-6xl mx-auto px-5 sm:px-8 py-16 sm:py-20">
      <SectionHead
        label="Experience"
        ...
        {education.map((e) => (
          <div key={e.school} className="card rounded-lg p-5">
            <div className="flex items-baseline justify-between gap-3">
              <h3 className="text-[15px] font-semibold m-0">{e.school}</h3>
              <span className="mono text-[10.5px] shrink-0" style={{ color: "var(--faint)" }}>
                {e.period}
              </span>
            </div>
            <p className="text-[13.5px] m-0 mt-1" style={{ color: "var(--muted)" }}>
              {e.degree}
            </p>
            <p className="mono text-[11px] m-0 mt-1.5" style={{ color: "var(--accent)" }}>
              {e.grade}
            </p>
            <p className="text-[12px] leading-relaxed m-0 mt-3" style={{ color: "var(--faint)" }}>
              {e.courses.join(" · ")}
            </p>
          </div>
        ))}
